...
PARAM_PREFIX = "return 1 / ((mark(\"PSmartMet_UP_8\") * "
PARAM_SUFFIX = " * 1.000000) + (mark(\"PSmartMet_SAFE_MO_10\") * "


PARAM_SUFFIX2 = " * 1.000000));"

PARAM_NAME = "UploadTOUExecTime"
...
def execute_sensitivity_analysis():
    ...
    new_value = MIN_VALUE
    values = list(range(1,106,5)) + list(range(120,220,20))

    for i in values:
        replace_value_in_file(str(i))
        execute_spnp()
        parse_spnp_report(i)
        new_value = i
...
def replace_value_in_file(new_value):
    ...
    for line in copy_file:
        ...
        if PARAM_PREFIX in line:
            tmp_line = PARAM_PREFIX + new_value + PARAM_SUFFIX + new_value + PARAM_SUFFIX2 + "\n"
